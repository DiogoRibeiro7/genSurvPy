...
def _print_summary(
    ...
    print("\nEVENT INFORMATION:")
    print(
        f"  Events:       {summary['event_info']['n_events']} "
        + f"({summary['event_info']['event_rate']:.1%})"
    )
    print(
        f"  Censored:     {summary['event_info']['n_censored']} "
    ...
    print(
        f"  Range:        {summary['time_info']['min']:.2f} to {summary['time_info']['max']:.2f}"
    )
    ...
    if quality_issues == 0:
        ...
        if summary["data_quality"]["missing_status"] > 0:
            print(
                f"    - Missing status values: {summary['data_quality']['missing_status']}"
            )
        if summary["data_quality"]["negative_time"] > 0:
            print(
                f"    - Negative time values: {summary['data_quality']['negative_time']}"
            )
        if summary["data_quality"]["invalid_status"] > 0:
            print(
                f"    - Invalid status values: {summary['data_quality']['invalid_status']}"
            )
