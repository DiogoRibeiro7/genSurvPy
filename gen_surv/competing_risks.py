...
def gen_competing_risks(
    ...
    if baseline_hazards is None:
        ...
        if len(baseline_hazards) != n_risks:
            raise ValueError(f"Expected {n_risks} baseline hazards, got {len(baseline_hazards)}")
    ...
    if betas is None:
        ...
        if betas.shape[0] != n_risks:
            raise ValueError(f"Expected {n_risks} sets of coefficients, got {betas.shape[0]}")
...
def gen_competing_risks_weibull(
    ...
    if shape_params is None:
        ...
        if len(shape_params) != n_risks:
            raise ValueError(f"Expected {n_risks} shape parameters, got {len(shape_params)}")
    ...
    if scale_params is None:
        ...
        if len(scale_params) != n_risks:
            raise ValueError(f"Expected {n_risks} scale parameters, got {len(scale_params)}")
    ...
    for j in range(n_risks):
        ...
        adjusted_scale = scale_params[j] * np.exp(-linear_predictors[:, j] / shape_params[j])
...
def cause_specific_cumulative_incidence(
    ...
    if cause not in unique_causes:
        raise ValueError(f"Cause {cause} not found in the data. Available causes: {unique_causes}")
...
def competing_risks_summary(
    ...
    if covariate_cols is None:
        covariate_cols = [col for col in data.columns 
                          if col not in [time_col, status_col, "id"]]
...
def plot_cause_specific_hazards(
    ...
    for cause in causes:
        ...
        at_risk = np.array([
            len(data[data[time_col] >= t]) for t in time_points
        ])
