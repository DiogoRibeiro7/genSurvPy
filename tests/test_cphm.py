...
def test_gen_cphm_covariate_range():
    ...
    covar_max = 2.5
    df = gen_cphm(n=100, model_cens="uniform", cens_par=1.0, beta=0.5, covar=covar_max)
    assert (df["covariate"] >= 0).all()
    assert (df["covariate"] <= covar_max).all()


def test_gen_cphm_seed_reproducibility():
    """Test that setting the same seed produces identical results."""
    df1 = gen_cphm(n=10, model_cens="uniform", cens_par=1.0, beta=0.5, covar=2.0, seed=42)
    df2 = gen_cphm(n=10, model_cens="uniform", cens_par=1.0, beta=0.5, covar=2.0, seed=42)
...
def test_gen_cphm_different_seeds():
    ...
    df1 = gen_cphm(n=10, model_cens="uniform", cens_par=1.0, beta=0.5, covar=2.0, seed=42)
    df2 = gen_cphm(n=10, model_cens="uniform", cens_par=1.0, beta=0.5, covar=2.0, seed=43)
