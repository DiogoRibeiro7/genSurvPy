...
def test_gen_cphm_output_shape():
    ...
    df = gen_cphm(
        n=50, model_cens="uniform", cens_par=1.0, beta=0.5, covariate_range=2.0
    )
...
def test_gen_cphm_status_range():
    ...
    df = gen_cphm(
        n=100, model_cens="exponential", cens_par=0.8, beta=0.3, covariate_range=1.5
    )
...
def test_gen_cphm_covariate_range():
    ...
    covar_max = 2.5
    df = gen_cphm(
        n=100, model_cens="uniform", cens_par=1.0, beta=0.5, covariate_range=covar_max
    )
    assert (df["X0"] >= 0).all()
...
def test_gen_cphm_seed_reproducibility():
    ...
    df1 = gen_cphm(
        n=10, model_cens="uniform", cens_par=1.0, beta=0.5, covariate_range=2.0, seed=42
    )
    df2 = gen_cphm(
        n=10, model_cens="uniform", cens_par=1.0, beta=0.5, covariate_range=2.0, seed=42
    )
...
def test_gen_cphm_different_seeds():
    ...
    df2 = gen_cphm(
        n=10, model_cens="uniform", cens_par=1.0, beta=0.5, covariate_range=2.0, seed=43
    )
